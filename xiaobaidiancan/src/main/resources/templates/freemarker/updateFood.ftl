...
<div style="margin-left:400px;margin-top: 90px;" id="app">
    ...
    <input v-model="fphoto" type="hidden" id="foodPhoto">
    <input v-model="fid" type="hidden" id="fid">
	<div class="layui-upload" style="margin-left: 70px;">
	  <button type="button" class="layui-btn" id="upload">上传图片</button>
	  <div class="layui-upload-list" >
	    <img src="${food.fphoto}" class="layui-upload-img" id="foodPic" style="width: 80px;height: 80px;">
	    <p id="demoText"></p>
	  </div>
	</div>
	
	<button id="addFood" type="button" class="layui-btn" style="margin-left: 70px;" v-on:click="updateFood">修改</button>
</div>
	  ...
	  methods:{
		  updateFood:function(){
			  ...
			  params.append("fphoto",fphoto);
			   ...
			   axios.post("updateFood",params).then(hnit=>{
				  if(hnit.data.code==-1){
					  layer.open({
						  content: "修改失败",
						  offset: 'rb'
